...
| **Program Outcome (PO)** | Broad professional skills and engineering graduate attributes defined by accreditation bodies like BAETE (`PO1` to `PO12`). |
...
| **Bounding Box (BBox)** | A set of rectangular coordinate tuples (`[ymin, xmin, ymax, xmax]`) demarcating the spatial location of words, questions, figures, or answer regions. |
...
| **LANCZOS Downsampling** | High-quality image resampling algorithm used in `LocalOfflineVisionProvider` to constrain image dimensions to max 800px before local inference. |
| **Composite Database Index** | Multi-column B-tree index in SQLite/PostgreSQL (e.g. `(examination, status)`, `(tabulation, student_id)`) optimizing multi-filter queries to O(1) execution. |
