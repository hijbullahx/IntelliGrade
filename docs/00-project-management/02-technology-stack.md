...
Cache & Session       Django LocMemCache                  5.2.x          OTP lifecycle management for password reset
...
- **Failover Chain**: Designed with a prioritized failover hierarchy:
  1. `LocalOfflineVisionProvider` (Moondream2 on CPU / Ollama) -> Zero API cost, operates offline.
  2. `GroqProvider` (`llama-3.3-70b-versatile`) -> Instant text evaluation with sub-second latency.
  3. `OpenRouterProvider` -> Cloud aggregator fallback to Mistral, Claude, or DeepSeek models.
  4. `GeminiProvider` (`gemini-2.5-flash` / `gemini-2.0-flash`) -> High-accuracy multimodal vision evaluation.
  5. `OpenAIProvider` (`gpt-4o` / `gpt-4o-mini`) -> Ultimate high-reasoning fallback.
- **Resilience Engine**:
  - `ProviderHealthTracker`: Tracks HTTP 429 rate limit events, placing providers on non-blocking cooldown timers (120s).
...
- Compiles real-time student grade records into an institutional OBE spreadsheet:
  ...
  - `CO_CLASS_ATTAINED` & `PO_CLASS_ATTAINED`: Class-wide OBE attainment thresholds (e.g. % of students scoring >= 50%).
